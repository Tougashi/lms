...
async function test() {
  ...
  const progRes = await fetch('https://lms-express-api-o5uk.vercel.app/api/v1/siswa/progress/cmprsw9t3001ho8ss696pq7bx', {
    headers: { 'Cookie': Array.isArray(cookies) ? cookies.join('; ') : cookies }
  });
  const progData = await progRes.json();
  console.log(JSON.stringify(progData, null, 2));
}

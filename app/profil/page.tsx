...
export default function ProfilPage() {
    ...
    useEffect(() => {
        ...
        siswaCertificateApi
            ...
            .then((res) => {
                if (isMounted)
                    setCertificates(extractArray<CertificateItem>(res));
            })
            .catch(() => {
                /* ignore */
            })
    ...
    const handleSigFileSelect = useCallback((file: File | null) => {
        ...
        if (!file.type.includes("png")) {
            setSigMsg("Hanya file .PNG yang diizinkan.");
            return;
        }
        if (file.size > 2 * 1024 * 1024) {
            setSigMsg("Ukuran file melebihi 2MB.");
            return;
        }
        setSigMsg("");
        setSigFile(file);
    ...
    const handleSigUpload = useCallback(async () => {
        ...
        try {
            ...
            setTutorProfile((prev) =>
                prev ? { ...prev, signatureUrl: res.signatureUrl } : prev,
            );
        } catch (err: unknown) {
            setSigMsg(
    ...
    const handleProfileFileSelect = useCallback((file: File | null) => {
        ...
        if (!file.type.startsWith("image/")) {
            setProfileUploadMsg("Hanya file gambar yang diizinkan.");
            return;
        }
        if (file.size > 5 * 1024 * 1024) {
            setProfileUploadMsg("Ukuran file melebihi 5MB.");
            return;
        }
        setProfileUploadMsg("");
        setProfileFile(file);
    ...
    const handleProfileUpload = useCallback(async () => {
        ...
        try {
            ...
            setProfile((prev) =>
                prev ? { ...prev, profileImage: res.url } : prev,
            );
    ...
    }, [profileFile, user, editName, editJenjang, editKelas, profile, updateUser]);

    const handleSaveProfile = useCallback(async () => {
        if (!profile) return;
        if (!editKelas.trim()) {
            setSaveMsg("Pilih tingkat kelas terlebih dahulu");
            return;
        }
        setIsSaving(true);
        setSaveMsg("");
        try {
            ...
            await authApi.update({
                ...
                ...(isTutor
                    ? {}
                    : { jenjang: editJenjang, kelas_sekolah: editKelas }),
                email: profile.email,
                password: "",
            });
            setProfile((prev) =>
    ...
    }, [editJenjang, editKelas, editName, profile, user, updateUser]);
    ...
    const displayName =
        profile?.nama_lengkap || user?.nama_lengkap || user?.fullName || "User";
    ...
    return (
        <div className="min-h-screen bg-[#f7f6fb] text-[#202126]">
            ...
            <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
                <h1 className="mb-4 text-lg font-bold text-[#202126]">
                    Profil
                </h1>
                ...
                <section className="rounded-2xl border border-[#e4e2eb] bg-white p-4 sm:p-6">
                    <div className="grid grid-cols-1 gap-5 md:grid-cols-[220px_1fr] md:gap-0">
                        ...
                        <div className="min-h-[560px] rounded-xl bg-white p-4 sm:p-5 md:rounded-none md:pl-6">
                            ...
                            {activeMenu === "profil" && (
                                <>
                                    <h2 className="text-base font-bold text-[#202126]">
                                        Profil Saya
                                    </h2>
                                        ...
                                        <>
                                            <div className="mt-4 rounded-xl border border-[#e4e2eb] bg-white px-4 py-3">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    {profileUploadMsg && (
                                                        <p
                                                            className={`mt-2 text-xs ${profileUploadMsg.includes("berhasil") ? "text-green-600" : "text-red-500"}`}
                                                        >
                                                            {profileUploadMsg}
                                                        </p>
                                                    )}
                                            ...
                                            <div className="mt-4 rounded-xl border border-[#e4e2eb] bg-white p-4">
                                                <div className="flex items-center justify-between gap-3">
                                                    <h3 className="text-sm font-semibold text-[#202126]">
                                                        Informasi Personal
                                                    </h3>
                                                    <button
                                                        type="button"
                                                ...
                                                {saveMsg && (
                                                    <p
                                                        className={`mt-2 text-sm ${saveMsg.includes("berhasil") ? "text-green-600" : "text-red-500"}`}
                                                    >
                                                        {saveMsg}
                                                    </p>
                                                )}
                                                ...
                                                {isEditing ? (
                                                    <div className="mt-4 space-y-4">
                                                        <div>
                                                            <label className="mb-1.5 block text-xs text-[#7d8291]">
                                                                Nama Lengkap
                                                            </label>
                                                            <input
                                                                type="text"
                                            ...
                                            {user?.role === "tutor" && (
                                                <div className="mt-4 rounded-xl border border-[#e4e2eb] bg-white p-4">
                                                    ...
                                                    <p className="mt-1 text-xs text-[#7d8291]">
                                                        ...
                                                        pada sertifikat
                                                        kelulusan siswa. Format
                                                        .PNG, latar transparan.
                                                    </p>

                                                    {sigMsg && (
                                                        <p
                                                            className={`mt-2 text-xs ${sigMsg.includes("berhasil") ? "text-green-600" : "text-red-500"}`}
                                                        >
                                                            {sigMsg}
                                                        </p>
                                                    )}
                                                    ...
                                                    >
                                                        {sigPreview ? (
                                                            <div className="flex flex-col items-center gap-2">
                                                                ...
                                                                />
                                                                <span className="text-[11px] text-[#7054dc]">
                                                                    Klik untuk
                                                                    mengganti
                                                                </span>
                                                            </div>
                                        ...
                                        {isCertLoading ? (
                                            <div className="mt-4 animate-pulse space-y-3">
                                                {[1, 2, 3].map((i) => (
                                                    <div
                                                        key={i}
                                                        className="h-16 w-full rounded-lg bg-[#f0eeff]"
                                                    />
                                                ))}
                                            </div>
                            ...
                            {activeMenu === "password" && (
                                <>
                                    ...
                                    <div className="mt-4 w-full max-w-[460px] rounded-xl border border-[#e4e2eb] bg-white p-4">
                                        {passwordMsg.text && (
                                            <p
                                                className={`mb-3 text-sm ${passwordMsg.isError ? "text-red-500" : "text-green-600"}`}
                                            >
                                                {passwordMsg.text}
                                            </p>
                                        )}
                                        <div className="space-y-4">
                                            <div>
                                                <label className="mb-1.5 block text-sm text-[#6b7080]">
                                                    Kata Sandi Lama
                                                </label>
                                                <div className="relative">
                                                    <input

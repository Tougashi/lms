...
function PrePostTestPageContent() {
            ...
            try {
                ...
                if (!pretest || !pretest.id) {
                    try {
                        ...
                        console.error("Gagal membuat pretest:", err);
                    }
                }
                if (pretest && pretest.id) {
    ...
    const handleAddQuestion = () => {
        ...
        if (isTestComputationalThinking) {
            const nid = Date.now();
            const groupId = `ct_${nid}`;
            const story: LocalCTStory = {
                id: nid,
                groupId,
                cerita: "",
                isExpanded: true,
                subQuestions: ctSubLabels.map((label, i) => ({
                    ...
                    answers: [
                        ...
                        { id: nid + i * 10 + 101, text: "", isCorrect: false },
                    ],
                    skor: 10,
                })),
            };
    ...
    const handleSaveCTStory = useCallback(
        async (story: LocalCTStory) => {
            ...
            if (!story.cerita.trim()) {
                toast("Cerita CT tidak boleh kosong.", "warning");
                return;
            }
            for (const sq of story.subQuestions) {
                ...
                if (filled.length < 2) {
                    toast(
                        `Setiap sub-soal minimal 2 pilihan jawaban.`,
                        "warning",
                    );
                    return;
                }
                ...
                if (!correct || !correct.text.trim()) {
                    toast(
                        `Setiap sub-soal harus memiliki jawaban benar.`,
                        "warning",
                    );
                    return;
                }
            ...
            try {
                ...
                for (const sq of story.subQuestions) {
                    ...
                    if (sq.apiSoalId) {
                        await (api as any).updateSoal(sq.apiSoalId, payload);
                    } else {
                        const result = await (api as any).addSoal(payload);
                        if (result?.id) {
                            updatedSubIds[sq.id] = result.id;
                        }
                    }
                }
                ...
                console.error("Save CT story error:", err);
                toast(
                    err instanceof Error
                        ? err.message
                        : "Gagal menyimpan soal CT.",
                    "error",
                );
            } finally {
                hideLoading();
                    ...
                    <div className="grid w-full lg:grid-cols-[260px_1fr] lg:items-stretch lg:min-h-[calc(100vh-74px)]">
                        ...
                        <section className="px-4 pb-8 pt-6 sm:px-6 lg:pr-6">
                            <h1 className="text-[18px] font-semibold text-[#232530]">
                                Pre Test dan Post Test Modul anda
                            </h1>
                            <p className="mt-2 max-w-[620px] text-[12px] leading-[1.6] text-[#7e8290]">
                                Buatlah Pre test dan Post Test Modul anda. anda
                                bisa menggunakan setiap Quis dari Topik yang
                                anda buat di konten modul atau bisa membuat yang
                                baru untuk soal ini.
                            </p>
                            <div className="mt-6">
                                <p className="text-[14px] font-semibold text-[#232530]">
                                    Nama Bank Soal
                                </p>
                                            ...
                                            >
                                                <div className="flex flex-wrap items-center justify-between gap-3">
                                                        ...
                                                        {editingSoalId ===
                                                            ...
                                                            <>
                                                                <input
                                                                    ...
                                                                    className="h-[30px] w-[240px] rounded-md border border-[#d9d7df] bg-white px-2 text-[12px] text-[#232530] outline-none focus:border-[#7054dc]"
                                                                    onKeyDown={(
                                                                        e,
                                                                    ) => {
                                                                        if (
                                                    ...
                                                    >
                                                        <FiChevronDown
                                                            size={16}
                                                            className={`transition-transform ${question.isExpanded ? "rotate-180" : ""}`}
                                                        />
                                                    </button>
                                                </div>
                                                    ...
                                                    <div className="mt-4">
                                                        ...
                                                        <div className="mt-4 flex items-center gap-3">
                                                            ...
                                                            <input
                                                                ...
                                                                min={0}
                                                                max={100}
                                                                className="h-[32px] w-[70px] rounded-lg border border-[#d9d7df] bg-white px-2 text-center text-[12px] text-[#232530] outline-none focus:border-[#7054dc]"
                                                            />
                                                        ...
                                                        {/* Answer options */}
                                                        <p className="mb-2 mt-4 text-[12px] font-semibold text-[#232530]">
                                                            Pilihan Jawaban
                                                        </p>
                                        ...
                                        <div className="mt-5 flex items-center justify-between border-b border-[#f0eff5] pb-4">
                                            <div>
                                                ...
                                                <p className="mt-1 text-[11px] text-[#7a7e8a]">
                                                    Semua soal (pretest,
                                                    posttest, quiz) akan mengacu
                                                    pada kategori Computational
                                                    Thinking
                                                </p>
                                            </div>
                                            <button
                                                ...
                                                className={`relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors ${isTestComputationalThinking ? "bg-[#7054dc]" : "bg-[#d1d5db]"}`}
                                            >
                                                <span
                                                    className={`inline-block h-4 w-4 rounded-full bg-white transition-transform ${isTestComputationalThinking ? "translate-x-6" : "translate-x-1"}`}
                                                />
                                            </button>
                                        </div>
            ...
            <main className="w-full">
                ...
                <div className="grid w-full lg:grid-cols-[260px_1fr] lg:items-stretch lg:min-h-[calc(100vh-74px)]">
                    ...
                    <section className="px-4 pb-8 pt-6 sm:px-6 lg:pr-6">
                        ...
                        <p className="mt-2 max-w-[620px] text-[12px] leading-[1.6] text-[#7e8290]">
                            Buatlah Pree test dan Post Test Modul anda. anda
                            bisa menggunakan setiap Quis dari Topik yang anda
                            buat di konten modul atau bisa membuat yang baru
                            untuk soal ini.
                        </p>
                                ...
                                <div className="w-full max-w-[500px] rounded-2xl bg-white px-6 py-5 shadow-[0_12px_40px_rgba(0,0,0,0.12)]">
                                    ...
                                    <div className="mt-5 flex items-center justify-between border-b border-[#f0eff5] pb-4">
                                        <div>
                                            ...
                                            <p className="mt-1 text-[11px] text-[#7a7e8a]">
                                                Semua soal (pretest, posttest,
                                                quiz) akan mengacu pada kategori

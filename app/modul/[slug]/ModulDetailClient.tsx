...
export default function ModulDetailPage({
    ...
    useEffect(() => {
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                if (topikWithMateri.length > 0)
                    setOpenSection(topikWithMateri[0].id);
    ...
    const handleEnroll = async () => {
        ...
        try {
            ...
            // Enroll berhasil, langsung ke halaman materi
            window.location.href = `/modul/${id}/materi`;
        } catch (err: unknown) {
            if (err instanceof ApiError && err.status === 409) {
                // Sudah terdaftar ΓÇö langsung ke materi
                window.location.href = `/modul/${id}/materi`;
                return;
            }

            // Backend kadang mengembalikan error tapi sebenarnya enroll sudah berhasil.
            // Coba cek progress untuk memastikan apakah sudah terdaftar.
            try {
                const prog = await siswaProgressApi.getByModul(id);
                if (prog) {
                    // Ternyata sudah berhasil terdaftar, langsung ke materi
                    window.location.href = `/modul/${id}/materi`;
                    return;
                }
            } catch {
                // Progress juga gagal ΓÇö mungkin memang belum terdaftar
            }

            // Coba enroll sekali lagi setelah jeda singkat
            try {
                ...
                if (retryErr instanceof ApiError && retryErr.status === 409) {
                    // Sudah terdaftar dari percobaan pertama
                    window.location.href = `/modul/${id}/materi`;
                    return;
                }
                setEnrollError(
                    ...
                    <div className="mx-auto max-w-7xl px-4 pb-24 pt-8 sm:px-6 sm:pb-28">
                        <div className="flex flex-col gap-6 rounded-3xl bg-transparent lg:flex-row lg:items-start">
                            ...
                            <div className="flex-1">
                                <h1 className="mb-8 mt-4 text-3xl font-bold text-[#202126]">
                                    {getModuleName(moduleData)}
                                </h1>
                                ...
                                {moduleData.rating != null && (
                                    <div className="mt-3 flex items-center gap-2 text-sm text-[#202126]">
                                        ...
                                        <span className="font-medium text-[#202126]">
                                            {Number(moduleData.rating).toFixed(
                                                1,
                                            )}
                                        </span>
                                    </div>
                                ...
                                <div className="mt-3 flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-[#202126]">
                                    ...
                                    {moduleData.target_waktu ||
                                    moduleData.targetTime ? (
                                        <span className="inline-flex items-center gap-2">
                                            <MdTimer
                                                size={16}
                                                className="text-[#7054dc]"
                                                                ...
                                                                .map((sub) => {
                                                                    const cleanTitle =
                                                                        sub.judul
                                                                            ? sub.judul
                                                                                  .replace(
                                                                                      /\s*[-ΓÇô]\s*[c][a-z0-9]{4,24}$/gi,
                                                                                      "",
                                                                                  )
                                                                                  .replace(
                                                                                      /\b[c][a-z0-9]{4,24}\b/gi,
                                                                                      getModuleName(
                                                                                          moduleData,
                                                                                      ),
                                                                                  )
                                    ...
                                    {progress.pretestScore != null && (
                                        <p className="inline-flex items-center gap-2">
                                            <FaCheck
                                                ...
                                                size={12}
                                            />
                                            Pre-Test: {progress.pretestScore}
                                            /100
                                        </p>
                                    )}
                                    {progress.posttestScore != null && (
                                        <p className="inline-flex items-center gap-2">
                                            <FaCheck
                                                ...
                                                size={12}
                                            />
                                            Post-Test: {progress.posttestScore}
                                            /100
                                        </p>
                                    )}

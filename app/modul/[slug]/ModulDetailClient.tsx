  ...
  const handleEnroll = async () => {
    ...
    try {
      ...
      try {
        ...
        setEnrollError(retryErr instanceof Error ? retryErr.message : 'Gagal mendaftar modul');
                        ...
                        {isOpen && topik.materi && topik.materi.length > 0 && (
                          <div className="space-y-3 border-t border-[#e7e4f2] px-4 py-3">
                            {topik.materi.flatMap((m) => m.submateris || m.submateri || []).map((sub) => {
                              const cleanTitle = sub.judul
                                ? sub.judul
                                    .replace(/\s*[-ΓÇô]\s*[c][a-z0-9]{4,24}$/gi, '')
                                    .replace(/\b[c][a-z0-9]{4,24}\b/gi, getModuleName(moduleData))
                                    .trim()
                                : '';
                              return (

...
import {
  FiCheckSquare, FiChevronDown, FiEdit2,
...
function EditModulPrePostContent() {
      ...
      try {
        ...
        if (pretestResult?.id) {
          ...
          try {
            ...
            const createdSettings = (created as any).pretestSettings;
            if (createdSettings?.length) {
              setSettingsDuration(createdSettings[0].duration ?? 90);
              setSettingsSoalTampil(createdSettings[0].countShownQuestions ?? 30);
            }
          } catch { /* auto-create failed */ }
        }

        // Process posttest
        if (posttestResult?.id) {
          setPosttestId(posttestResult.id);
          const pSettings = (posttestResult as any).posttestSettings;
          if (pSettings?.length) {
            setPosttestDuration(pSettings[0].duration ?? 90);
            setPosttestCountShown(pSettings[0].countShownQuestions ?? 30);
          }
        }
  ...
  const handleDeleteBank = useCallback(async () => {
    ...
    try {
      await adminPretestApi.delete(bank.apiId);
    } catch (err: unknown) {
  ...
  const handleAddCTStory = () => {
    if (!bank) return;
    setBank((prev) => prev ? { ...prev, ctStories: [...prev.ctStories, makeCTStory()] } : prev);
  };
  ...
  const handleDeleteQuestion = useCallback(async (qId: number) => {
    ...
    if (question?.apiSoalId) {
      try {
        await adminPretestApi.deleteSoal(question.apiSoalId);
      } catch (err: unknown) {
        toast(err instanceof Error ? err.message : 'Gagal menghapus soal.', 'error'); return;
      }
    }
    setBank((prev) => prev ? { ...prev, questions: prev.questions.filter((q) => q.id !== qId) } : prev);
  }, [bank, toast]);
  ...
  const handleDeleteCTStory = useCallback(async (storyId: number) => {
    ...
    const story = bank.ctStories.find((s) => s.id === storyId);
    if (story) {
      for (const sq of story.subQuestions) {
        if (sq.apiSoalId) {
          try { await adminPretestApi.deleteSoal(sq.apiSoalId); } catch { /* ignore */ }
        }
      }
    }
    setBank((prev) => prev ? { ...prev, ctStories: prev.ctStories.filter((s) => s.id !== storyId) } : prev);
  }, [bank]);

  const handleToggleCorrect = (qId: number, aId: number) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id !== qId ? q : { ...q, answers: q.answers.map((a) => ({ ...a, isCorrect: a.id === aId })) }),
    } : prev);
  };
  const handleRemoveAnswer = (qId: number, aId: number) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id !== qId ? q : { ...q, answers: q.answers.filter((a) => a.id !== aId) }),
    } : prev);
  };
  const handleAddAnswer = (qId: number) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id !== qId ? q : { ...q, answers: [...q.answers, { id: Date.now(), text: '', isCorrect: false }] }),
    } : prev);
  };
  const handleUpdateQuestionText = (qId: number, text: string) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id === qId ? { ...q, pertanyaan: text } : q),
    } : prev);
  };
  const handleUpdateSkor = (qId: number, skor: number) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id === qId ? { ...q, skor } : q),
    } : prev);
  };
  const handleUpdateAnswerText = (qId: number, aId: number, text: string) => {
    setBank((prev) => prev ? {
      ...prev, questions: prev.questions.map((q) => q.id !== qId ? q : { ...q, answers: q.answers.map((a) => a.id === aId ? { ...a, text } : a) }),
    } : prev);
  };
  ...
  const handleSaveQuestion = useCallback(async (question: LocalQuestion) => {
    if (!bank?.apiId) { toast('Bank soal belum tersimpan.', 'error'); return; }
    ...
    try {
      ...
      if (question.apiSoalId) {
        await adminPretestApi.updateSoal(question.apiSoalId, payload);
      } else {
        await adminPretestApi.addSoal({ pretest_id: bank.apiId, ...payload });
      }
      setBank((prev) => prev ? {
        ...prev, questions: prev.questions.map((q) => q.id === question.id ? { ...q, isExpanded: false } : q),
      } : prev);
    ...
    try {
      ...
      for (const sq of story.subQuestions) {
        const payload: any = {
          ...
          pretest_id: bank.apiId,
        };

        if (sq.apiSoalId) {
          await adminPretestApi.updateSoal(sq.apiSoalId, payload);
        } else {
          const result: any = await adminPretestApi.addSoal(payload);
          if (result?.id) updatedSubIds[sq.id] = result.id;
        }
      }

      setBank((prev) => prev ? {
        ...prev, ctStories: prev.ctStories.map((s) => s.id !== story.id ? s : {
          ...s,
          isExpanded: false,
  ...
  const handleSaveSettings = useCallback(async () => {
    ...
    try {
      await Promise.all([
        ...
        posttestId ? adminPosttestApi.updateSettings(posttestId, { duration: posttestDuration, countShownQuestions: posttestCountShown }) : Promise.resolve(),
      ]);
  ...
  }, [bank, posttestId, settingsDuration, settingsSoalTampil, posttestDuration, posttestCountShown, toast]);
  ...
  const SettingsModal = () => (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-full max-w-[500px] rounded-2xl bg-white px-6 py-5 shadow-[0_12px_40px_rgba(0,0,0,0.12)]">
        ...
        <p className="mt-3 text-[12px] font-bold text-[#7054dc]">Pre Test</p>
        <div className="mt-3 flex items-center justify-between border-b border-[#f0eff5] pb-4">
          <div><p className="text-[13px] font-semibold text-[#232530]">Durasi Pengerjaan (Menit)</p><p className="mt-1 text-[11px] text-[#7a7e8a]">Batas waktu siswa untuk menyelesaikan Pre Test</p></div>
        ...
        <div className="mt-4 flex items-center justify-between border-b border-[#f0eff5] pb-4">
          <div><p className="text-[13px] font-semibold text-[#232530]">Jumlah Soal Tampil</p><p className="mt-1 text-[11px] text-[#7a7e8a]">Jumlah soal acak dari bank soal Pre Test</p></div>
        ...
        <p className="mt-4 text-[12px] font-bold text-[#7054dc]">Post Test</p>
        <div className="mt-3 flex items-center justify-between border-b border-[#f0eff5] pb-4">
          <div><p className="text-[13px] font-semibold text-[#232530]">Durasi Pengerjaan (Menit)</p><p className="mt-1 text-[11px] text-[#7a7e8a]">Batas waktu siswa untuk menyelesaikan Post Test</p></div>
          <div className="flex items-center gap-2"><input type="number" value={posttestDuration} onChange={(e) => setPosttestDuration(parseInt(e.target.value) || 0)} className="h-[32px] w-[60px] rounded-lg border border-[#d9d7df] bg-white px-2 text-center text-[12px] outline-none" /><span className="text-[12px] text-[#7a7e8a]">Menit</span></div>
        </div>
        <div className="mt-4 flex items-center justify-between border-b border-[#f0eff5] pb-4">
          <div><p className="text-[13px] font-semibold text-[#232530]">Jumlah Soal Tampil</p><p className="mt-1 text-[11px] text-[#7a7e8a]">Jumlah soal acak dari bank soal Post Test</p></div>
          <div className="flex items-center gap-2"><input type="number" value={posttestCountShown} onChange={(e) => setPosttestCountShown(parseInt(e.target.value) || 0)} className="h-[32px] w-[60px] rounded-lg border border-[#d9d7df] bg-white px-2 text-center text-[12px] outline-none" /><span className="text-[12px] text-[#7a7e8a]">Soal</span></div>
        </div>
    ...
    <div className="min-h-screen bg-[#f7f6fb] text-[#232530]">
      ...
      <main className="flex w-full">
        ...
        <section className="flex-1 px-4 pb-10 pt-6 sm:px-6 lg:px-8">
          ...
          {/* Warnings */}
          {modulId && (() => {
            const hasQuestions = bank ? bank.questions.length > 0 : false;
            const hasCT = bank ? bank.ctStories.length > 0 : false;
            const warnings: string[] = [];
            if (isTestComputationalThinking) {
              if (!hasCT) warnings.push("Mode CT sedang Aktif: Anda belum memiliki soal Pre Test CT. Harap tambahkan bank soal Pre Test dan isi dengan soal CT.");
            } else {
              if (!hasQuestions) warnings.push("Mode CT sedang Nonaktif: Anda belum memiliki soal Pre Test Reguler. Harap tambahkan bank soal Pre Test dan isi dengan soal reguler.");
            }
          ...
          {bank && (
            <div className="mt-6 rounded-2xl border border-[#e5e3ee] bg-white px-5 py-4">
              <div className="flex items-center gap-3">
                <p className="text-[13px] font-semibold text-[#232530]">{bank.name}</p>
                <span className="rounded-full bg-[#ece7ff] px-2 py-0.5 text-[10px] font-bold text-[#7054dc]">Pre Test</span>
                <button type="button" onClick={() => setIsSettingsOpen(true)} className="text-[#7a7e8a] hover:text-[#7054dc]"><FiSettings size={14} /></button>
                <button type="button" onClick={handleDeleteBank} className="ml-auto text-[#7a7e8a] hover:text-red-500"><FiTrash2 size={14} /></button>
              </div>
            </div>
          )}
            ...
            {bank?.questions.map((question, idx) => (
              <div key={question.id} className="rounded-2xl border border-[#e5e3ee] bg-white px-5 py-4">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setBank((prev) => prev ? { ...prev, questions: prev.questions.map((q) => q.id === question.id ? { ...q, isExpanded: !q.isExpanded } : q) } : prev)} className="inline-flex h-6 w-6 items-center justify-center rounded-md text-[#7a7e8a] hover:bg-[#f5f4fb]">
                ...
                <div key={story.id} className="rounded-2xl border-2 border-[#c8b8f8] bg-white px-5 py-4">
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      <button type="button" onClick={() => setBank((prev) => prev ? { ...prev, ctStories: prev.ctStories.map((s) => s.id === story.id ? { ...s, isExpanded: !s.isExpanded } : s) } : prev)} className="inline-flex h-6 w-6 items-center justify-center rounded-md text-[#7a7e8a] hover:bg-[#f5f4fb]">

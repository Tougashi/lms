...
import {
  FiArrowLeft, FiCheck, FiChevronDown, FiImage, FiX,
} from 'react-icons/fi';
...
function CustomSelect({ value, onChange, options, placeholder = '— Pilih —', disabled = false, error }: CustomSelectProps) {
  ...
  return (
    <div ref={ref} className="relative mt-1.5" onBlur={handleBlur}>
      ...
      >
        <span className={!value ? 'text-[#c0bfca]' : ''}>{selected ? selected.label : placeholder}</span>
        <FiChevronDown size={15} className={`text-[#9b97ad] transition-transform duration-150 ${open ? 'rotate-180' : ''}`} />
      </button>
      {open && !disabled && (
            ...
            <li key={opt.value}>
              <button type="button" onClick={() => { onChange(opt.value); setOpen(false); }}
                className={['flex w-full items-center justify-between px-4 py-2.5 text-[13px] transition-colors',
                  value === opt.value ? 'bg-[#f0edfb] text-[#7054dc] font-semibold' : 'text-[#232530] hover:bg-[#f7f5ff]',
                ].join(' ')}>
                {opt.label}
                {value === opt.value && <FiCheck size={13} className="text-[#7054dc]" />}
              </button>
            </li>
          ))}
        </ul>
...
function pillBtn(active: boolean) {
  return [
    ...
    active ? 'bg-[#7054dc] text-white' : 'bg-white text-[#6b6880] hover:bg-[#f5f2ff]',
  ].join(' ');
}
...
function EditModulContent() {
  ...
  const featureCards = [
    ...
    { id: 'prepost', label: 'Pre / Post Test', checked: pretestPostTestEnabled, setter: setPretestPostTestEnabled },
    { id: 'cert', label: 'Sertifikat', checked: hasCertificate, setter: setHasCertificate },
    { id: 'group', label: 'Grup Belajar', checked: hasStudyGroup, setter: setHasStudyGroup },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#f7f6fb] text-[#232530]">
      ...
      <main className="flex w-full">
        ...
        <section className="flex-1 px-4 pb-12 pt-6 sm:px-6 lg:px-8">
          <div className="mx-auto max-w-3xl space-y-5">
              ...
              >
                {coverPreview ? (
                  ...
                  <div className="flex h-full w-full flex-col items-center justify-center gap-3 border-2 border-dashed border-[#d0cce8] bg-[#f7f6fb] rounded-xl">
                    ...
                    <p className="text-[13px] text-[#a0a3af]">Klik untuk upload cover modul</p>
                    <p className="text-[11px] text-[#c0bfca]">Rekomendasi: 1280 × 720 px (16:9)</p>
                  </div>
                )}
                {coverUploading && (
            ...
            <div className="rounded-2xl border border-[#edeaf6] bg-white p-6 shadow-[0_2px_12px_rgba(112,84,220,0.06)]">
              ...
              {/* Judul Modul */}
              <div>
                <label className={labelCls}>Judul Modul <span className="text-[#e8473f]">*</span></label>
                <input
                  type="text"
                  value={moduleName}
                  onChange={(e) => { setModuleName(e.target.value); setErrors((p) => ({ ...p, moduleName: '' })); }}
              ...
              {/* Subtitle */}
              <div className="mt-4">
                <label className={labelCls}>Subtitle <span className="text-[#e8473f]">*</span></label>
                <input
                  type="text"
                  value={subtitle}
                ...
                {errors.subtitle
                  ...
                  : <p className={hintCls}>Gunakan 1 atau 2 kata kunci terkait, sebutkan area terpenting yang dibahas.</p>
                }
              </div>
              ...
              <div className="mt-4">
                ...
                <div className="mt-1 flex items-center justify-between">
                  <p className={hintCls}>Deskripsikan modul anda secara singkat</p>
              ...
              {/* Guru Modul */}
              <div className="mt-4">
                <label className={labelCls}>Guru Modul <span className="text-[#e8473f]">*</span></label>
                <CustomSelect
                  value={tutorId}
                  onChange={(v) => { setTutorId(v); setErrors((p) => ({ ...p, tutorId: '' })); }}
                ...
                {errors.tutorId
                  ...
                  : <p className={hintCls}>Pilih guru yang bertanggung jawab atas modul ini.</p>
                }
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-[#edeaf6] bg-white p-6 shadow-[0_2px_12px_rgba(112,84,220,0.06)]">
              ...
              {accessType === 'siswa' && (
                <div className="mt-4 grid gap-4 sm:grid-cols-2">
                  <div>
                    ...
                    <CustomSelect value={level} onChange={setLevel} options={jenjangOptions} placeholder="Pilih Jenjang" />
                  ...
                  <div>
                    ...
                    <CustomSelect
                      ...
                      placeholder={level ? 'Pilih Tingkatan Kelas' : 'Pilih jenjang dulu'}
                      disabled={!level}
                    />
              ...
              <div className="mt-4 grid gap-4 sm:grid-cols-2">
                <div>
                  ...
                  <CustomSelect value={difficulty} onChange={setDifficulty} options={difficultyOptions} placeholder="Pilih Level" />
                  <p className={hintCls}>Level kesulitan yang sesuai isi modul.</p>
                </div>
                <div>
                  ...
                  <div className="mt-1.5 flex gap-2">
                    ...
                    <div className="flex-1">
                      <CustomSelect
                        ...
                        onChange={(v) => setTargetTimeUnit(v as 'bulan' | 'minggu')}
              ...
              <div className="mt-4">
                ...
                <div className="mt-1.5 inline-flex overflow-hidden rounded-xl border border-[#e2e0ea]">
                  <button type="button" className={pillBtn(!isDraft)} onClick={() => setIsDraft(false)}>Aktif</button>
                  <button type="button" className={pillBtn(isDraft)} onClick={() => setIsDraft(true)}>Draft</button>
                </div>
                <p className={hintCls}>Draft tidak akan tampil di halaman publik.</p>
              </div>
            </div>
            ...
            <div className="rounded-2xl border border-[#edeaf6] bg-white p-6 shadow-[0_2px_12px_rgba(112,84,220,0.06)]">
              ...
              <div className="grid grid-cols-2 gap-3">
                {featureCards.map(({ id: fid, label, checked, setter }) => (
                  <button
                    key={fid}
                    type="button"
                    onClick={() => setter(!checked)}
                    className={['flex cursor-pointer items-center gap-3 rounded-xl border p-3.5 text-left transition-colors',
                      checked ? 'border-[#7054dc] bg-[#f5f2ff]' : 'border-[#e2e0ea] bg-white hover:border-[#c8c4db]',
                    ].join(' ')}
                  >
                    <span className={['flex h-[18px] w-[18px] shrink-0 items-center justify-center rounded-md border-2 transition-colors',
                      checked ? 'border-[#7054dc] bg-[#7054dc]' : 'border-[#c8c4db] bg-white',
                    ].join(' ')}>
                      {checked && <FiCheck size={11} className="text-white" strokeWidth={3} />}
                    </span>
                    <span className={['text-[13px] font-semibold', checked ? 'text-[#7054dc]' : 'text-[#3d3a4a]'].join(' ')}>
                      {label}
                    </span>
                  </button>
                ))}

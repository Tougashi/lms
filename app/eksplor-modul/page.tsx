...
function getPreTestLabel(item: EnrolledModuleItem): string {
  const score = item.pretestScore ?? item.progress?.pretestScore;
...
export default function EksplorModulPage() {
  ...
  const [enrolledModules, setEnrolledModules] = useState<EnrolledModuleItem[]>([]);
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        setRelevantModules(allModules.filter((m) => !enrolledIds.has(m.id)));
  ...
  const filteredModules = useMemo(() => {
    const source = activeTab === 'terdaftar' ? enrolledModules : relevantModules;
    ...
    return source.filter((item) => {
      ...
      if (activeTab !== 'terdaftar' || activeStatus === 'semua') return true;

      const status = getStatusFromEnrolled(item as EnrolledModuleItem);
      if (activeStatus === 'dalam-progress') return status === 'dalam-progress' || status === 'belum-mulai';
      if (activeStatus === 'selesai') return status === 'selesai';
      return true;
    });
  }, [activeStatus, activeTab, enrolledModules, relevantModules, searchQuery]);
      ...
      <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6 sm:py-8">
        ...
        {isLoading ? (
          <div className="grid gap-4 md:grid-cols-2">
            {[1, 2, 3, 4].map((i) => (
              <div key={i} className="animate-pulse rounded-2xl border border-[#d8d9e0] bg-white p-4 sm:p-5">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {filteredModules.map((item) => {
              ...
              const enrolledItem = isEnrolled ? (item as EnrolledModuleItem) : null;
              const progress = enrolledItem ? getProgress(enrolledItem) : 0;
              const status = enrolledItem ? getStatusFromEnrolled(enrolledItem) : null;

              return (
                <article key={item.id} className="relative rounded-2xl border border-[#d8d9e0] bg-white p-4 sm:p-5 transition-shadow hover:shadow-md">
                  {activeTab === 'relevan' && (
                  ...
                  <div className={`flex items-center gap-3 ${activeTab === 'relevan' ? 'pr-7 sm:gap-4' : ''}`}>
                    ...
                    <div className="min-w-0 flex-1">
                      <h3 className="text-xl font-semibold text-[#202126]">{getModuleName(item)}</h3>
                      <p className="mt-1 text-xs text-[#60636d]">{getJenjangKelas(item)}</p>

                      {enrolledItem ? (
                        <div className="mt-4 flex items-end gap-3">
                          <div className="min-w-0 flex-1">
                            <p className="text-[11px] font-medium text-[#f39b39]">{getPreTestLabel(enrolledItem)}</p>
                            <div className="mt-2 flex items-center gap-2">
                              ...
                              <span className="text-[11px] text-[#4f5565]">{Math.round(progress)}% Progress</span>
                            </div>
                            {status === 'selesai' && (
                              <span className="mt-1.5 inline-flex items-center gap-1 text-[10px] font-semibold text-[#f39b39]">
                                <FaBookOpen size={9} /> Selesai
                              </span>
                            )}
                          </div>
                          ...
                          >
                            {status === 'selesai' ? 'Ulangi' : 'Lanjut'}
                          </Link>
                        </div>
                      ) : (
                        <div className="mt-4 flex items-center justify-between gap-3">
                          <p className="truncate text-xs text-[#202126] sm:text-sm">{getTutorName(item)}</p>
                          <Link
                            href={`/modul/${item.id}`}
